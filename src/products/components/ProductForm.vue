      ...
      <form>
        ...
        >
          <b-input
            ...
            @input="() => errors.productName = []"
            type="text"
          ></b-input>
        </b-field>
        <b-field label="Вес продукта" :type="formData.weight ? 'is-success' : ''">
        ...
        >
          ...
          >
            <b-input
              placeholder="Введи цену в формате: 50.00"
        ...
        <b-field label="Описание продукта" :type="formData.content ? 'is-success' : ''">
          <b-input
        ...
        <b-field label="Состав продукта" :type="formData.ingridients ? 'is-success' : ''">
          <b-input
            placeholder="Состав продукта"
            v-model="formData.ingridients"
            type="textarea"
            :rows="5"
          ></b-input>
        </b-field>
        <b-field label="Начинки">
          <div id="sorts" class="is-flex">
            <b-checkbox
              ...
              :native-value="filling.label"
              type="is-success"
            >{{ filling.label }}</b-checkbox>
          </div>
        </b-field>
      </form>
    </div>
      ...
      <div class="columns is-multiline">
        ...
        <div class="column is-full is-half-fullhd">
          ...
          <div v-if="currentRoute === 'UpdateProduct'" class="field">
            <div class="control">
              <router-link
                :to="`/products-list/${formData.category}`"
...
import { CreateProduct, UpdateProduct, DeleteProduct } from '../graphql'
...
export default {
    ...
    watch: {
        $route() {
            ...
            if (this.$route.name === 'UpdateProduct') {
                this.setData(this.getProductById)
            }
        }
    },
    data() {
    ...
    methods: {
        setData: function(productData) {
            this.formData = setFormData(this.formData, productData)
        ...
        pushProductToDB: async function() {
            try {
                const product = makeModel(this.formData)
                const result =
                ...
                if (result.data.createProduct) {
                    this.$store.commit({
                        ...
                        item: result.data.createProduct
                    })
